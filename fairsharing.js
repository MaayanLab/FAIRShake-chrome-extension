...
$("head").append('<style type="text/css">div.tooltip {' +
    ...
    'max-width: 300px;' +
    '}</style>');
$("head").append('<style type="text/css">#insigform{ position:relative; padding-top:5px; margin-top:0px; top:0px; max-height:50px;}</style>');

$(document).ready(function () {

    //don't need refresh

    var theURL = $(".col-md-8").children('p').eq(0).find('a').text();

    var theDescrip = $(".description-text").text();
    var theName = $("h2").text();
    ...
    var kokobop = $(".col-md-8").children("div").eq(2);
    var printex = $(kokobop).find("p").text();

    $(kokobop).before('<div><p><span class="heavier">FAIRness Assessment</span></p>'
        ...
        + '<input type="hidden" name="theSrc" value="Fairsharing">'
        + '<label id="submitlabel"><input type="submit" id="submithere"/></label></form></div>');
});
